...
3. شغّل سكربت SQL الجاهز في ملف `sql/users_schema_fix.sql` من خلال **SQL Editor** في لوحة Supabase (الصورة التوضيحية في طلبك هي القائمة المطلوبة). السكربت يبدأ بإزالة سياسات RLS القديمة مؤقتًا عشان ما تظهرش رسالة الخطأ "cannot alter type of a column used in a policy definition"، ثم يضبط الأنواع والربط مع `auth.users`، يضيف قيود فريدة على البريد، يفعل RLS، وينشئ السياسات اللازمة من جديد. الإصدار الحالي يعتمد على العمود الصحيح `policyname` في `pg_policies` لتفادي خطأ "column \"polname\" does not exist" أثناء الحذف وإعادة الإنشاء.
